...
def draw_tdcr_try(g: np.ndarray[float], seg_end: np.ndarray[int], r_disk: float=2.5*1e-3, r_height: float=1.5*1e-3, tipframe: bool=True, segframe: bool=False, baseframe: bool=False, projections: bool=False, baseplate: bool=True):
...
if "__main__" == __name__:
    ...
    # draw_tdcr_try(np.array(data["onesegtdcr"]), np.array([10]), tipframe=True, segframe=True, baseframe=True, projections=True, baseplate=True)
    ...
    draw_tdcr_try(np.array(data["foursegtdcr"]), np.array([15, 30, 45, 60]), tipframe=True, segframe=True, baseframe=True, projections=True, baseplate=True)